...
mkdir -p "${SSL_DIR}" 2>/dev/null || {
	echo "Warning: Could not create ${SSL_DIR} (may not have permissions)"
...
# ssl_dhparam ${DHPARAM_FILE}; # Commented out - generate with: openssl dhparam -out dhparam.pem 2048
...
# DH parameter generation (skip for faster container startup)
# DH params are optional for modern TLS configurations
echo "Skipping Diffie-Hellman parameters generation for faster startup..."
echo "Note: For production, pre-generate dhparam.pem with: openssl dhparam -out dhparam.pem 2048"

echo "Creating SSL configuration..."
create_ssl_config

echo "Setting certificate permissions..."
chmod 644 "${CERT_FILE}" "${SSL_CONFIG_FILE}"
chmod 600 "${KEY_FILE}"
[ -f "${DHPARAM_FILE}" ] && chmod 644 "${DHPARAM_FILE}"
